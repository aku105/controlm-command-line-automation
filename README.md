...
|---|------|---|
|`create_node_groups.sh` | Reads through a properties file (example: `nodegroups_create.properties`) and create node groups. | ```./create_node_groups.sh PATH_TO_PROPERTIES``` |
|`delete_node_groups.sh` | Deletes all node groups in the properties file (example: `nodegroups_delete.properties`). | ```./delete_node_groups.sh PATH_TO_PROPERTIES``` |
